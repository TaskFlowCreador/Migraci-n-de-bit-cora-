...
function ModuloAdmin({ showToast, setDialogConfig, socios, sociosCargando, cargarSocios }) {
  ...
  const guardarSocioDoc = async (s) => {
    await setDoc(doc(db, 'socios', String(s.num)), { num: s.num, nombre: s.nombre, cedula: s.cedula, pagos: s.pagos || {} });
  };
      ...
      <>
        ...
        {pagoModal && pagoModal.numSocio === socio.num && (
          <ModalPago
            ...
            onGuardado={async (nuevoSocio) => { await cargarSocios(); }}
...
function ModalPago({ socio, pagoModal, onClose, guardarSocioDoc, onGuardado, showToast }) {
  ...
  const guardar = async () => {
    ...
    const nuevoSocio = { ...socio, pagos: { ...(socio.pagos || {}) } };
    nuevoSocio.pagos[key] = {
    ...
    try {
      await guardarSocioDoc(nuevoSocio);
      onClose();
